...
/// True when a browser user agent is likely to delete stored sessions on its
/// own schedule — the WebKit crowd: Safari (ITP evicts script-written storage
/// after 7 days without interaction), Bluefy, and other iOS shells (their
/// `CriOS`/`FxiOS` tokens carry neither `Chrome/` nor `Firefox/`). Firefox is
/// excluded (evicts only under disk pressure, LRU); Chromium forks keep the
/// `Chrome/` token. Negative-match on purpose: a false positive costs an info
/// banner, a false negative costs sessions.
