...
void main() {
  ...
  testWidgets('Peak row follows the graph window', (tester) async {
    ...
    for (int i = 0; i < 1000; i++) {
      frame.fillRange(0, wireNumAdcChan, i < 500 ? 900 : 700);
    ...
    expect(find.text('+900'), findsNWidgets(wireNumAdcChan)); // Peak row
    expect(find.text('+700'), findsNWidgets(wireNumAdcChan)); // Live row
    ...
    expect(find.text('+700'), findsNWidgets(wireNumAdcChan * 2));
    ...
    expect(find.text('+900'), findsNWidgets(wireNumAdcChan));
    expect(find.text('+700'), findsNWidgets(wireNumAdcChan));
  });
}
