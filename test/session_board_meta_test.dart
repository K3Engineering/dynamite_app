...
void main() {
  ...
  group('SessionBoardMeta JSON', () {
    test('round-trips every field', () {
      ...
      expect(back.provisioned, meta.provisioned);
    ...
    test(
      ...
      () {
        const bare = SessionBoardMeta(provisioned: false, provenance: {});
        ...
        expect(back.provisioned, isFalse);
    ...
    test('a malformed provisioned flag is damage, not a guess', () {
      final json = meta.toJson()..['provisioned'] = 'maybe';
      expect(() => SessionBoardMeta.fromJson(json), throwsFormatException);
    });

    test('a missing provisioned flag is damage', () {
      final json = meta.toJson()..remove('provisioned');
      expect(() => SessionBoardMeta.fromJson(json), throwsFormatException);
    });
  ...
  group('SessionBoardMeta.fromBoard', () {
    ...
    test('an unprovisioned board snapshots the bare fact', () {
      final meta = SessionBoardMeta.fromBoard(
        const UnprovisionedBoardCalibration(),
      );
      expect(meta.provisioned, isFalse);
